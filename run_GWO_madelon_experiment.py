...
from gwo_algorithm import run_GWO
...
def run_single_experiment(seed, k_vecinos, alpha, pop_size, max_iter, 
    ...
    # (Log de iteraciones - sin cambios)
    log_data = []
    ...
    # Resultados finales (para estadísticas) - (ACTUALIZADO)
    final_results_summary = {
        ...
        'f1_full': f1_full,
        'precision_gwo': acc_subset,
        'recall_gwo': recall_subset,
        'f1_gwo': f1_subset,
        'num_features_gwo': num_features_seleccionadas,
        'rd_gwo': rd_doa, # Reducción de Dimensionalidad
...
if __name__ == "__main__":
    ...
    RESULTS_DIR = "results_madelon_GWO"
    ...
    for i in range(NUM_RUNS):
        ...
        if i == 0:
            ...
            # (Cambiado a .csv)
            log_csv_path = os.path.join(LOGS_DIR, "reporte_madelon_seed_0.csv")
            log_plot_path = os.path.join(LOGS_DIR, "graficos_madelon_seed_0.png")
            
            # (Cambiado a generate_csv_report)
            generate_csv_report(log_data, filename=log_csv_path)
            generate_plots(pd.DataFrame(log_data), plot_results, filename=log_plot_path, algoritmo="gwo")

    # Análisis Estadístico Final
    # (Cambiado a .csv)
    stats_csv_path = os.path.join(STATS_DIR, "reporte_estadistico_final_madelon.csv")
    stats_plot_path = os.path.join(STATS_DIR, "analisis_estadistico_boxplots_madelon.png")
    
    # (Cambiado a csv_filename)
